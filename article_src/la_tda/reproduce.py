...
def train(config):
    ...
    # We do it for the 4 language (Ru is not in the LA-TDA but in a following article) and also 4 new languages.
    # Namely, ZH, NO, JA and FR.
    for lang in [
        "en",
        "it",
        "ru",
        ...
        for seed in seeds:
            ...
            for subset in ["train", "dev", "test"]:
                ...
                # Process using 3/4 the number of CPU core plus 2 on the computer.
                # e.g. on 16 cores, it will use 14 cores.
                num_of_workers = int(os.cpu_count() // 4 * 3 + 2)

                batch_size = 10  # batch size for the features processing (per CPU core)
            ...
            (_, y_train), (_, y_valid), (_, y_test) = list(
                map(
        ...
        with open(os.path.join(".", f"reproduce_results_time_{lang}.txt"), "w") as file:
            print(
                f"Average time taken to process the features over the 10 runs for all three splits is: "
                f"{mean(processing_time)}",
                file=file,
