...
def count_template_features(
    ...
    logger.info(
        f"Used token_ids while computing the distance-to-pattern features:"
...
def compute_template_features(
    ...
    **kwargs,
    ...
    logger.debug(
        f"Loading {data_path.suffix[1:]} dataset from path: {str(data_path.as_posix())}..."
    )
    dataset = load_dataset_tsv(data_files={subset: data_path.as_posix()})
    logger.debug(
        "Getting `n_tokens` of passed sentences. Using `sentence` column of the dataset."
    )
    ...
    features_array = []
    comma_id, dot_id = list(
        map(lambda x: tokenizer.convert_tokens_to_ids(x), [",", "."])
    )
    ...
    for i, filename in enumerate(tqdm(adj_filenames, desc="Template Feature Calc")):
        ...
        sentences = np.array(
            dataset[subset]["sentence"][i * batch_size : (i + 1) * batch_size]
        )
        logger.debug("Sentence samples: " + "\n".join(sentences[:2]))
        splitted_indexes = np.array_split(np.arange(batch_size), num_of_workers)
        splitted_list_of_ids = [
            get_list_of_ids(
                tokenizer, sentences[indx], max_seq_length=max_seq_length, **kwargs
            )
            for indx in splitted_indexes
        ]
        splitted_adj_matricies = [adj_matricies[indx] for indx in splitted_indexes]
        t_args = [
            (m, feature_list, (comma_id, dot_id, sep_id), list_of_ids)
            for m, list_of_ids in zip(splitted_adj_matricies, splitted_list_of_ids)
        ]
        features_array_part = pool.starmap(calculate_features_t, t_args)
    ...
    templates_file = str(features_dir) + f"/{subset}_template" + ".npy"
    ...
    # Close the pool
    pool.close()  # releasing resources of ripser
    pool.terminate()  # terminate the process
    pool.join()  # release resources
    del pool
    gc.collect()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_file", type=str, help="A csv datafile; required columns: `sentence`."
    )
    parser.add_argument(
        "--attn_dir",
        default="",
        type=str,
        help="A directory with model attention weights saved.",
    )
