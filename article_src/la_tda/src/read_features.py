...
topological_feature_names = [
    f"{n}_t{t}" for n in topological_names for t in topological_thresholds
]
...
template_feature_names = ["self", "beginning", "prev", "next", "comma", "dot", "sep"]
...
def load_features(
    ...
    """
    topological_thrs = "_".join([topological_features, "array", str(topological_thr)])
    ...
    pbar = tqdm.tqdm(
        total=len(all_features_dict) * 12 * layers,
        desc="Loading {} features...".format(dataset_name),
        position=0,
        leave=True,
    )
    Xs = []
    for feature_type, read_args in all_features_dict.items():
        ...
        for layer, head in itertools.product(range(12), range(layers)):
            ...
            Xs.append(
                X
            )  # https://stackoverflow.com/questions/35367882/python-fast-dataframe-concatenation
            pbar.update(1)
    pbar.close()
    features = pd.concat(Xs, axis=1)

    return features


def read_labels(set_name, data_dir="./data", file_type=".tsv"):
    header = "infer"
    names = None
    delimeter = ","
    if "tsv" in file_type:
        delimeter = "\t"
    df = pd.read_csv(
        os.path.join(data_dir, set_name + file_type),
        header=header,
        names=names,
        delimiter=delimeter,
    )
