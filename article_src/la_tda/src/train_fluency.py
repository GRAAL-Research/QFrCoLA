...
class DataTrainingArguments:
    ...
    hold_out_file: Optional[str] = field(
        default=None,
        metadata={"help": "A csv or a json file containing the hold out data."},
    )
    wandb_project: Optional[str] = field(
        default="la-tda",
        metadata={"help": "Name of the Wandb project to use during training."},
    )
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        if training_args.do_predict:
            ...
            if data_args.hold_out_file is not None:
                train_extension = data_args.train_file.split(".")[-1]
                hold_out_extension = data_args.hold_out_file.split(".")[-1]
                assert (
                    hold_out_extension == train_extension
                ), "`test_file` should have the same extension (csv or json) as `train_file`."
                data_files["hold_out"] = data_args.hold_out_file
    ...
    if training_args.do_eval:
        ...
        if data_args.max_predict_samples is not None:
            max_test_samples = min(len(test_dataset), data_args.max_predict_samples)
            test_dataset = test_dataset.select(range(max_test_samples))

        if raw_datasets.get("hold_out") is not None:
            hold_out_dataset = raw_datasets["hold_out"]
            if data_args.max_predict_samples is not None:
                max_test_samples = min(
                    len(hold_out_dataset), data_args.max_predict_samples
                )
                hold_out_dataset = hold_out_dataset.select(range(max_test_samples))
    ...
    if added_args.balance_loss:
        ...
        label_weights = {
            id: weight
            for weight, id in zip(
                np.unique(raw_datasets["train"]["label"]), class_weights
            )
        }
    ...
    # Evaluation
    if training_args.do_eval:
        logger.info("*** Evaluate on eval and test set***")
