...
def grab_attention_weights(
    ...
    **kwargs,
    ...
    inputs = tokenizer.batch_encode_plus(
        ...
        # XLM-RoBERTa does not, by default, return the token types https://github.com/huggingface/transformers/issues/2702
...
def grab_attention_weights_inference(
    model_dir,
    data_file,
    batch_size=10,
    max_seq_length=64,
    ...
    **kwargs,
    ...
    adj_matricies = []
    assert number_of_batches == len(batched_sentences), "Batch Split Error"
    attn_dir = pathlib.Path(model_dir + "/attentions")
    attn_dir.mkdir(parents=True, exist_ok=True)
    for i in tqdm(range(number_of_batches), desc="Weights Extraction"):
        attention_w = grab_attention_weights(
            model, tokenizer, batched_sentences[i], max_seq_length, device, **kwargs
        )
    ...
    cleanup_memory(trainer=None, model=model, tokenizer=tokenizer)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract attention weights.")
    parser.add_argument(
        "--model_dir", type=str, help="A directory with model weights saved."
    )
    parser.add_argument(
        "--no_cuda",
        action="store_true",
    ...
    parser.add_argument(
        "--data_file",
        type=str,
        help="A csv datafile; required columns: `sentence` or `text`",
    )
