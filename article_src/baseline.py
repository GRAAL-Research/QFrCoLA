...
def predict(row):
    return {"prediction": [1] * len(row["sentence"])}
...
for lang in [
    ...
    dev_dataset = dev_dataset.map(predict, batched=True, batch_size=64)
    ...
    with open(f"{lang}-bloom.txt", "w") as file:
        ...
        if lang == "fr":
            ...
            ood_dataset = ood_dataset.map(predict, batched=True, batch_size=64)
