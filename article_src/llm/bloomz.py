...
for lang in [
    ...
    dev_dataset = dev_dataset.map(
        pipe_fn, batched=True, batch_size=batch_size, desc="bloomz-dev"
    )
    ...
    test_dataset = test_dataset.map(
        pipe_fn, batched=True, batch_size=batch_size, desc="bloomz-test"
    )
    ...
    with open(os.path.join(root_dir, f"{lang}-bloomz.txt"), "w") as file:
        ...
        if lang == "fr":
            ...
            ood_dataset = ood_dataset.map(
                pipe_fn, batched=True, batch_size=batch_size, desc="bloomz-ood"
            )
